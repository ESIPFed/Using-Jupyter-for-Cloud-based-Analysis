...
do
  docker run -d --rm --name $(echo trmm_$g | cut -d'-' -f 1) --network docker_nexus -v ${NINGESTER_CONFIG}/connections.yml:/config/connections.yml -v ${NINGESTER_CONFIG}/trmm.yml:/config/trmm.yml -v ${DATA_DIRECTORY}/${g}:/data/${g} sdap/ningester:${VERSION} docker,solr,cassandra
  sleep 15
done
